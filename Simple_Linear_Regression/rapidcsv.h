//
// Created by Ashrita on 23-Nov-21.
//

#ifndef ML_MULTIPLE_REGRESSION_RAPIDCSV_H
#define ML_MULTIPLE_REGRESSION_RAPIDCSV_H
...
namespace rapidcsv
{
    ...
    long double my_stod(const std::string &valueAsString) {
        std::istringstream totalSString( valueAsString );

        totalSString.precision(std::numeric_limits<long double>::max_digits10);
        totalSString.flags((std::ios_base::fmtflags(std::ios_base::scientific)));
    ...
    public:
        ...
        void ToVal(const std::string& pStr, T& pVal) const
        {
            ...
            try
            {
                ...
                {
                    pVal = static_cast<T>(my_stod(pStr));
//          pVal = static_cast<T>(std::stold(pStr));
//            std::cout<<std::setprecision(10)<<" "<<pStr<<" "<<pVal<<std::endl;
                    return;
                }
            }
            catch (...)
...
#endif //ML_MULTIPLE_REGRESSION_RAPIDCSV_H
